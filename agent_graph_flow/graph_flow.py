...
from typing import Any, Callable, Deque, Dict, List, Literal, Mapping, Sequence, Set, Union, Optional
...
from autogen_agentchat.messages import BaseAgentEvent, BaseChatMessage, ChatMessage, MessageFactory, StopMessage, TextMessage
...
_DIGRAPH_STOP_AGENT_MESSAGE = "AgentDigraph execution is complete"
...
class DiGraphEdge(BaseModel):
    ...
    check_message_key: Optional[str] = None
    check_message_value: Optional[str] = None
    ...
    async def check_condition_message(self, message: BaseChatMessage) -> bool:
        try:
            if self.check_message_key is not None and self.check_message_value is not None:
                message_content_dict = json.loads(message.to_model_text())
                return message_content_dict.get(self.check_message_key) = self.check_message_value
            else:
                return True
        except:
            return True
    ...
    async def update_message_thread(self, messages: Sequence[BaseAgentEvent | BaseChatMessage]) -> None:
        ...
        for edge in self._edges[source]:
            ...
            if not edge.check_condition(message):
                continue
            if not await edge.check_condition_message(message):
                continue
